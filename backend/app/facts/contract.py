...
def identity_fields(fact):
    """Select identity components, without normalization or hashing (TAXO-01B)."""
    validate_fact(fact, submission=False)
    identity = {key: deepcopy(fact[key]) for key in _IDENTITY_FIELDS[fact['kind']] if key in fact}
